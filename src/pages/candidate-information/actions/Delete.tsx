...
import { deleteCandidate } from "../../../services/candidate_information";
...
function Delete(props: DeleteProductProps) {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      notification.success('Xóa CV thành công')
      onCancel();
      ...
      if (err instanceof Error) {
        ...
        if (err.message === "Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại.") {
          ...
          notification.error('Xóa CV thất bại')
        }
      }
    } finally {
      setLoading(false);
  ...
  return (
    <Modal
      ...
      wrapClassName='!p-0'
    >
      <div className="w-full text-center p-3 h-[60px] leading-[36px] bg-[#24652c] rounded-t-lg uppercase font-bold">{`Bạn muốn xóa CV này không`}</div>
